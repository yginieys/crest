...
server.post('/:db/:collection', function (req, res) {
  ...
  if (req.params) {
    MongoClient.connect(util.connectionURL(req.params.db, config), function (err, db) {
      ...
      collection.insert(Array.isArray(req.params) ? util.cleanParams(req.params[0]) : util.cleanParams(req.params), function (err, createdDocs) {
        var result = util.flavorize(createdDocs[0], "out");
        res.json(result, {'content-type': 'application/json; charset=utf-8'});
        db.close();
      });
    });
  ...
  MongoClient.connect(util.connectionURL(req.params.db, config), function (err, db) {
    db.collection(req.params.collection, function (err, collection) {
      collection.update(spec, util.cleanParams(req.params), true, function (err, docs) {
        collection.findOne(spec, function(err, modifiedDoc) {
          var result = util.flavorize(modifiedDoc, "out");
          res.json(result, {'content-type': 'application/json; charset=utf-8'});
          db.close();
        });
      });
